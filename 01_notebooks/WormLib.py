...
def embryo_segmentation(bf, image_ch3, image_name, output_directory):
    ...
    # Calculate embryo area in pixels
    embryo_area_pixels = np.sum(masks_cytosol > 0)

    area_df = pd.DataFrame({
        "Image ID": [image_name],
        "embryo_area_pixels": [embryo_area_pixels]
    })

    area_path = os.path.join(output_directory, f"{image_name}_embryo_area.csv")
    ...
    return masks_cytosol, masks_nuclei, masks_cyto_sizes, masks_nuc_sizes, embryo_area_pixels

if run_embryo_segmentation:
    masks_cytosol, masks_nuclei, masks_cyto_sizes, masks_nuc_sizes, embryo_area_pixels = embryo_segmentation(
...
calculate_tif_mask_colocalization = False
