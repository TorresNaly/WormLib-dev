...
# # # Note: input directory = image subdiorectory
# folder_name = '/pl/active/onishimura_lab/PROJECTS/naly/bigfish/01_quant-paper/Carolines_unseen_data/4-cell/input/111324_1038'
...
def matching_masks(masks_cytosol, masks_nuclei):
    n_mask_cyto = np.max(masks_cytosol)
    n_mask_nuc = np.max(masks_nuclei)
    new_masks_nuclei = np.zeros_like(masks_cytosol)
    reordered_mask_nuclei = np.zeros_like(masks_cytosol)
    if n_mask_cyto > 0 and n_mask_nuc > 0:
        for mc in range(1, n_mask_cyto + 1):
            tested_mask_cyto = np.where(masks_cytosol == mc, 1, 0)
...
#     MIN_CELL_SIZE = 1000
#     list_masks_nuclei = []
#     list_thresholds = np.arange(0.7, 0.95, 0.05)  # thresholds to test
...
#     # Calculate mask sizes (number of pixels per nucleus)
#     mask_sizes_nuc = [np.sum(masks_nuclei == i) for i in range(1, np.max(masks_nuclei) + 1)]

#     return masks_nuclei, mask_sizes_nuc


def cytosol_segmentation(image_cytosol, second_image_cytosol=None, cytosol_diameter=300):
...
def segmentation(image_cytosol, image_nuclei, second_image_cytosol=None):
    ...
    # --- Threshold / diameter optimization loop ---
    list_flow_thresholds = np.arange(40, 200, 10)
    best_metric = -1
    best_masks_cytosol = None
    best_masks_nuclei = None
    best_list_masks_cyto_sizes = None
    best_list_masks_nuc_sizes = None
    best_outline_masks = None
    best_outline_nuc = None

    for ts in list_flow_thresholds:
        try:
            # Cytosol segmentation
            masks_cytosol, list_masks_cyto_sizes, outline_masks = cytosol_segmentation(
                image_cytosol, second_image_cytosol, cytosol_diameter=ts
            )

            # Nuclear segmentation
            if image_nuclei is not None:
                masks_nuclei, list_masks_nuc_sizes = nuclear_segmentation(image_nuclei)
                masks_nuclei = matching_masks(masks_cytosol, masks_nuclei)
                masks_nuclei = remove_lonely_masks(masks_0=masks_nuclei, masks_1=masks_cytosol, is_nuc='nuc')
                masks_cytosol = remove_lonely_masks(masks_0=masks_cytosol, masks_1=masks_nuclei)
            ...
            # --- Metric to select best threshold ---
            metric = metric_max_cells_and_area(masks_cytosol)
            if masks_nuclei is not None:
                metric += metric_max_cells_and_area(masks_nuclei)

            if metric > best_metric:
                best_metric = metric
                best_masks_cytosol = masks_cytosol
                best_masks_nuclei = masks_nuclei
                best_list_masks_cyto_sizes = list_masks_cyto_sizes
                best_list_masks_nuc_sizes = list_masks_nuc_sizes
                best_outline_masks = outline_masks
                best_outline_nuc = outline_nuc

        except Exception as e:
            print(f"Segmentation failed at ts={ts}: {e}")
            continue

    # --- Use best results ---
    masks_cytosol = best_masks_cytosol
    masks_nuclei = best_masks_nuclei
    list_masks_cytosol_sizes = best_list_masks_cyto_sizes
    list_masks_nuc_sizes = best_list_masks_nuc_sizes
    outline_masks = best_outline_masks
    outline_nuc = best_outline_nuc
    ...
    return masks_cytosol, masks_nuclei, list_masks_cytosol_sizes, list_masks_nuc_sizes, fig
...
def embryo_segmentation(bf, image_nuclei, image_name, output_directory):
    ...
    model_nuclei = models.Cellpose(model_type='cyto2')
    masks_nuclei, _, _, _ = model_nuclei.eval(
        nuclei_image, diameter=70, channels=[0, 0],
        cellprob_threshold=0.0, flow_threshold=0.0
    )
...
# # --- Post-processing: remove small nuclei (e.g., midbody) ---
# labeled_nuc = label(masks_nuclei)
# props = regionprops(labeled_nuc)
# areas = np.array([p.area for p in props])
...
# if len(areas) > 0:
#     median_area = np.median(areas)
#     min_area_threshold = 0.5 * median_area  # remove masks smaller than 50% of median
