...
# # 1. Define image path, microscope parameters and channel names (set to None if the channel does not exist)
# folder_name = '/pl/active/onishimura_lab/PROJECTS/naly/bigfish/Steven_test_251020/input/04' #image subdirectory path
...
# Cy5 = None  # (Q670)
# mCherry = "translation"  # (Q610)
# FITC = "zif-1_mRNA"  # (GFP)
# DAPI = "DAPI"
# brightfield = "brightfield"
# channel_order = ["mCherry", "FITC", "DAPI"]
...
#             if not f.startswith('.ipynb_checkpoints')
#         ])
#         path_files = [os.path.join(folder_path, f) for f in list_filenames]

#         # Read DV files and store image stacks
#         list_images = [stack.read_dv(image) for image in path_files]
...
#     # Convert to uint16
#     bf = list_images[1].astype(np.uint16)  # Brightfield
#     image_colors = list_images[0].astype(np.uint16)  # Fluorescence channels

#     print(f'Image colors \n{list_filenames[0]}: {image_colors.shape}\n')
#     print(f'Brightfield \n{list_filenames[1]}: {bf.shape}')

#     # Determine number of channels
#     n_channels = image_colors.shape[0]

#     # Assign channels safely
#     Cy5_array = image_colors[0] if n_channels > 0 else None
#     mCherry_array = image_colors[1] if n_channels > 1 else None
#     FITC_array = image_colors[2] if n_channels > 2 else None
#     nuclei_array = image_colors[3] if n_channels > 3 else None

#     # Max projections only if the channel exists
#     image_Cy5 = np.max(Cy5_array, axis=0) if Cy5_array is not None else None
#     image_mCherry = np.max(mCherry_array, axis=0) if mCherry_array is not None else None
#     image_FITC = np.max(FITC_array, axis=0) if FITC_array is not None else None
#     image_nuclei = np.max(nuclei_array, axis=0) if nuclei_array is not None else None

#     bf = bf if brightfield is not None else None  # optional brightfield check
...
if dv_images:
    ...
    channel_order = ["mCherry", "FITC", "DAPI"]  # your 3 acquired channels
    n_channels = image_colors.shape[0]

    # ----------------------------------------------------------
    # Assign channels manually based on your known order
    # ----------------------------------------------------------
    mCherry_array = image_colors[0] if n_channels > 0 else None
    FITC_array    = image_colors[1] if n_channels > 1 else None
    nuclei_array  = image_colors[2] if n_channels > 2 else None

    # (No Cy5 channel in this dataset)
    Cy5_array = None
    ...
    # Combine and plot
    # ----------------------------------------------------------
    images = [image_mCherry, image_FITC, image_nuclei, bf]
    titles = ["mCherry", "FITC", "DAPI", "Brightfield"]
...
#         spots_post_clustering_ch0, clusters_ch0,list_spots_in_each_cell_ch0,list_clusters_in_each_cell_ch0 = spot_detection(rna_ch0,voxel_size,spot_radius_ch0,masks_cytosol)
    ...
    if FITC is not None and image_FITC is not None:
        rna_ch0 = FITC_array # [Z,Y,X,C]
        rna_channel = FITC
        detection_color = "red"
        spots_post_clustering_ch0, clusters_ch0,list_spots_in_each_cell_ch0,list_clusters_in_each_cell_ch0 = spot_detection(rna_ch0,voxel_size,spot_radius_ch0,masks_cytosol)
...
if any(x is not None for x in [sum_spots_ch0, sum_spots_ch1]):
    ...
    data_wide = {
        'Image ID': image_name,
        f'{FITC} total molecules': sum_spots_ch0,
    ...
    if run_cell_classifier and 'features_df' in locals():
        ...
        for i in range(num_cells):
            ...
            row = {
                'Image ID': image_name,
                f'{FITC}': list_spots_in_each_cell_ch0[i] if i < len(list_spots_in_each_cell_ch0) else None,
...
rna_names = [FITC, mCherry]  # Your actual mRNA names as strings
rna_images = [image_FITC, image_mCherry]  # Your actual images
...
# # Then, call the function as you wrote (correcting rna_names to strings)
# rna_names = [mCherry]  # Your actual mRNA names as strings
# rna_images = [image_mCherry]  # Your actual images
# colormap_list = ['PiYG']
# output_directory = output_directory  # your output directory variable
# image_name = image_name  # your image name variable
