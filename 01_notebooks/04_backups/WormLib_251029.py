...
# # 1. Define image path, microscope parameters and channel names (set to None if the channel does not exist)
# folder_name = '/pl/active/onishimura_lab/PROJECTS/naly/bigfish/01_quant-paper/01_N2_erm-1_set-3/240812_N2_erm-1_set-3/2-cell/input/08' #image subdirectory path
    ...
    if contours:
        ...
        if len(cnt) >= 5:  # At least 5 points needed to fit an ellipse
            ...
            scatter_plot_path = os.path.join(
                output_directory, f'{mRNA_name}_line_density_no_shading_{image_name}.png'
            )
            plt.savefig(scatter_plot_path, bbox_inches='tight', dpi=300)
            plt.show()
            plt.close()
