...
print(f"Feature selection: PSF={psf_calculator}, Embryo segmentation={run_embryo_segmentation}, Spot Detection={run_spot_detection}, Cell_segmentation={run_cell_segmentation}")
...
# # # Note: input directory = image subdiorectory
# folder_name = '/pl/active/onishimura_lab/PROJECTS/naly/bigfish/02_DG_quantification/wDMP07/240702_WDMP07_lin-41_set-3/input/4 cell/02'
...
if run_embryo_segmentation:
    ...
    masks_cytosol, flows_cytosol, styles_cytosol, diams_cytosol = model_cyto.eval(
        cytosol_image, diameter=275, channels=[0, 0]
    )
...
# #### 1.2 Single cell segmentation (up to 4-cell embryos)
...
#Apply threshold to protein intensity heatmap
...
#collecting all the images into a list
image_paths = []
...
#sorting the images by creation time
sorted_images = sorted(image_paths, key=lambda image: os.path.getctime(image))
sorted_images.reverse()


#Collecting Image Titles
imageTitles = []
for filenames in sorted_images:
    imageTitle = os.path.basename(filenames)
...
#Making new heights and widths of images based on aspect ratio
imageHeights = []
imageWidths = []
for aspect in aspectRatios:
    ...
    if width > 548:
       ...
       imageWidths.append(548)
    else:
       imageWidths.append(width)
       imageHeights.append(145.6)
...
y_orgin = 720 - imageHeights[0]
margin = 32
#letter = max Y coord 792 and max X coord 612
c = canvas.Canvas(output_pdf_path, pagesize=letter)
c.setFont("Times-Roman", 16)
c.drawString(margin, 728, f"Report Generated: {currentDate}")
for image_path, height, width in zip(sorted_images, imageHeights, imageWidths):
    c.drawImage(image_path, margin, y_orgin, width, height)
    addPageNumber(c)
    if y_orgin >= height + 10:
        y_orgin -= height + 10
    else: 
        c.showPage()
