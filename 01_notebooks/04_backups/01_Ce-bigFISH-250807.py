...
if run_cell_classifier: print("Cell classification")
...
# # # Note: input directory = image subdiorectory
# folder_name = '/pl/active/onishimura_lab/PROJECTS/naly/bigfish/erm-1_ERM-1-png-test/280829_erm-1_ERM-1/input/09'
...
if run_cell_classifier:
    ...
    # Predict cell type and add to dataframe
...
# Predict probabilities and get class labels
    ...
    for idx, row in features_df.iterrows():
        ...
        plt.text(x, y, pred_label, color='white', fontsize=16, ha='center', va='center', weight='bold')
...
# new_column_order = ['Image ID', 'cell_stage'] + [f'{Cy5} total molecules']  + [f'{mCherry} total molecules'] + cy5_cell_columns + mcherry_cell_columns
...
if any(x is not None for x in [sum_spots_ch0, sum_spots_ch1]):
    ...
    # Add per-cell information for Cy5 and mCherry
    for i, value in enumerate(list_spots_in_each_cell_ch0):
        data[f'{Cy5} cell_{chr(65 + i)}'] = value

    for i, value in enumerate(list_spots_in_each_cell_ch1):
        data[f'{mCherry} cell_{chr(65 + i)}'] = value

    # Add classifier results if enabled
    if run_cell_classifier:
        for i, row in features_df.iterrows():
            label = row["highest_confidence_label"]
            confidence = row["prediction_confidence"]
            data[f'cell_{chr(65 + i)}_label'] = label
            data[f'cell_{chr(65 + i)}_confidence'] = round(confidence, 3)

#     # Count nuclei to estimate stage
#     labeled_array = label(masks_nuclei)
...
#     data['cell_stage'] = num_features
    ...
    base_cols = ['Image ID', 'cell_stage', f'{Cy5} total molecules', f'{mCherry} total molecules']
    extra_cols = []

    if run_cell_classifier:
        for i in range(len(features_df)):
            extra_cols.extend([
                f'cell_{chr(65 + i)}_label',
                f'cell_{chr(65 + i)}_confidence'
            ])

    # Reorder columns
    new_column_order = base_cols + cy5_cell_columns + mcherry_cell_columns + extra_cols
    df_quantification = df_quantification.reindex(columns=new_column_order)

# Optional: show the final DataFrame
df_quantification
