...
# Feature selection 
# run_segmentation = os.getenv('SEGMENTATION', 'True') == 'True'
# run_spot_detection = os.getenv('SPOT_DETECTION', 'True') == 'True'
...
# # # Note: input directory = image subdiorectory
# folder_name = '/pl/active/onishimura_lab/PROJECTS/naly/bigfish/05-Ambika/wLPW004/wLPW004_240203_recoded/input/31'
...
if run_embryo_segmentation:
    ...
    masks_cytosol, flows_cytosol, styles_cytosol, diams_cytosol = model_cyto.eval(
        cytosol_image, diameter=300, channels=[0, 0]
    )
...
# # Pixel sizes
# pixel_size_xy = 0.108  # µm
# z_step_size = 2.0      # µm

# # Volume = count of voxels × voxel volume
# voxel_volume = pixel_size_xy ** 2 * z_step_size  # µm³

# volume_cytosol = np.sum(masks_cytosol > 0) * voxel_volume
# volume_nuclei  = np.sum(masks_nuclei  > 0) * voxel_volume

# print(f"Cytosol volume: {volume_cytosol:.2f} µm³")
# print(f"Nuclei volume:  {volume_nuclei:.2f} µm³")




# # Pixel size in microns (update to your actual values!)
# pixel_size_xy = 0.178  # e.g., 0.108 µm/pixel

# # Calculate area in µm²
# area_cytosol = np.sum(masks_cytosol > 0) * (pixel_size_xy ** 2)
# area_nuclei  = np.sum(masks_nuclei  > 0) * (pixel_size_xy ** 2)

# print(f"Cytosol area: {area_cytosol:.2f} µm²")
# print(f"Nuclei area:  {area_nuclei:.2f} µm²")
