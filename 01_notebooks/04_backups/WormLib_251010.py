...
# # 1. Define image path, microscope parameters and channel names (set to None if the channel does not exist)
# folder_name = '/pl/active/onishimura_lab/PROJECTS/naly/bigfish/01_quant-paper/01_N2_erm-1_set-3/240812_N2_erm-1_set-3/2-cell/input/11' #image subdirectory path
...
if run_2cell_classifier:
    ...
    print("Skipped 2-cell classifier")
...
if run_4cell_classifier:
   ...
   ## plot cell centroid position
    ...
    print("Skipped 4-cell classifier")


# #### 3.3 Embryo Segementation
...
if run_embryo_segmentation:
    ...
    print("Skipped embryo segmentation")
...
if any(x is not None for x in [sum_spots_ch0, sum_spots_ch1]):
    ...
    quantification_output = os.path.join(output_directory, f'total_counts_{image_name}.csv')
...
# #             ax.set_xlabel('AP Position normalized')
...
#             ap_positions = []
#             total_major = features_df['major_axis_length'].sum()
#             current_pos = 0
#             for _, row in features_df.iterrows():
#                 frac = row['major_axis_length'] / total_major * 100
#                 start = current_pos
...
def analyze_rna_density(image, masks_cytosol, colormap, mRNA_name, image_name, output_directory):
    ...
    if contours:
        ...
        if len(cnt) >= 5:  # At least 5 points needed to fit an ellipse
            ...
            # Always plot basic normalized intensity along AP axis
            fig, ax = plt.subplots()
            for i in range(len(positions)):
            ...
            # Only add AP-axis cell shading and mRNA counts if classifier was run
            if run_cell_classifier:
                fig, ax = plt.subplots()
                # AP-axis cell shading
                ap_positions = []
                ...
                for _, row in df_long.iterrows():
                    ...
                    if start is not None and end is not None:
                        ...
                        ax.text(mid, 0.9, f"{label}", ha='center', va='top', fontsize=20, fontweight='bold', transform=ax.get_xaxis_transform())
                        # mRNA count above
                        ax.text(mid, 0, f"{mRNA_count} \n total \n molecules", ha='center', va='bottom', fontsize=20, transform=ax.get_xaxis_transform())

                # Plot normalized intensity on top
                for i in range(len(positions)):
                    ax.scatter(positions[i], normalized_intensity[i], color=colormap_values[i], s=50, label=f'Grid {i}' if i == 0 else "")
                ax.plot(positions, normalized_intensity, color='gray', linestyle='-', linewidth=1)
                ax.set_xlabel('Position along Body Axis (% distance)')
