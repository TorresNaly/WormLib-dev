...
# run_line_scan = os.getenv('RUN_LINE_SCAN', 'True') == 'True'
...
# # Feature selection
# print(f"\nEnabled Features:")
# if run_embryo_segmentation: print("  Embryo segmentation")
...
# if analyze_rna_density: print("  RNA density analysis")
# if run_line_scan: print(" Line scan")
...
# # Note: input directory = image subdiorectory
folder_name = '/pl/active/onishimura_lab/PROJECTS/naly/bigfish/erm-1_ERM-1-png-test/280829_erm-1_ERM-1/input/03'
...
# # Get indices sorted by size (descending)
# sorted_indices = np.argsort(list_masks_cyto_sizes)[::-1]  # Largest first
...
#     label = idx + 1  # Mask labels usually start from 1
#     label_to_name[label] = cell_names[i]


# # Assume 2D mask (if 3D, use a projection or slice first)
# mask_image = masks_cytosol  # shape: (y, x)

# # Create color overlay (optional)
...
#     coords = center_of_mass(mask_image == label)
#     y, x = coords
...
# labeled_masks(masks_cytosol, list_masks_cyto_sizes)
...
if run_embryo_segmentation:
    # Assume bf is your cytosol image and image_nuclei is your nuclei image
    ...
    masks_cytosol, flows_cytosol, styles_cytosol, diams_cytosol = model_cyto.eval(
        cytosol_image, diameter=320, channels=[0, 0]
    )

    # Only keep the largest cytosol mask (the embryo)
    ...
    masks_nuclei, flows_nuclei, styles_nuclei, diams_nuclei = model_nuclei.eval(
    ...
    # Create binary mask for embryo_outline
    image_shape = cytosol_image.shape  # same shape as used to generate the outlines
    embryo_outline = np.zeros(image_shape, dtype=bool)

    for outline in outlines_cytosol:
        # Reverse to (y, x) for polygon2mask
        poly_mask = polygon2mask(image_shape, outline[:, ::-1])
        embryo_outline |= poly_mask  # Combine all outlines into one mask

    
    segmentation_filename = os.path.join(output_directory, 'embryo_segmentation_' + image_name + '.png')
...
if any(x is not None for x in [sum_spots_ch0, sum_spots_ch1]):
    ...
    if run_cell_segmentation:
        # Add per-cell info for Cy5
        for i, value in enumerate(list_spots_in_each_cell_ch0):
            cell_label = label_to_name.get(i + 1, f'cell_{i+1}')  # fallback label
            data[f'{Cy5} {cell_label}'] = value

        # Add per-cell info for mCherry
        for i, value in enumerate(list_spots_in_each_cell_ch1):
            cell_label = label_to_name.get(i + 1, f'cell_{i+1}')  # fallback label
            data[f'{mCherry} {cell_label}'] = value
    else:
        # Just total abundance for each channel
        data[f'{Cy5} total molecules'] = sum_spots_ch0
        data[f'{mCherry} total molecules'] = sum_spots_ch1
    ...
    data['cell_stage'] = num_features - 1
...
# # Use label_to_name keys for consistent ordering
if run_cell_segmentation:
    cy5_cell_columns = [f'{Cy5} {label_to_name.get(i + 1, f"cell_{i+1}")}' for i in range(len(list_spots_in_each_cell_ch0))]
    mcherry_cell_columns = [f'{mCherry} {label_to_name.get(i + 1, f"cell_{i+1}")}' for i in range(len(list_spots_in_each_cell_ch1))]
    ...
    # Only keep the essential columns when no cell segmentation
    new_column_order = ['Image ID', f'{Cy5} total molecules', f'{mCherry} total molecules']

# Reorganize the DataFrame columns
df_quantification = df_quantification.reindex(columns=new_column_order)
...
if run_mRNA_heatmaps:
    def create_heatmap(spots_x, spots_y, masks_cytosol, masks_nuclei, output_filename, title_suffix, grid_width=grid_width, grid_height=grid_height):
    ...
    if contours:
        ...
        if len(cnt) >= 5:  # At least 5 points needed to fit an ellipse
            ...
            if dot_product is not None and dot_product >= 0:
                ...
                colormap_values = colormap_values[::-1]
...
def run_line_scan(image, masks_cytosol, colormap, mRNA_name, image_name, output_directory):
    import numpy as np
    import cv2
    import matplotlib.pyplot as plt
    import matplotlib.patches as patches
...
for mRNA_name, image, colormap in zip(rna_names, rna_images, colormap_list):
    run_line_scan(image=image, masks_cytosol=masks_cytosol, colormap=colormap, mRNA_name=mRNA_name, image_name=image_name, output_directory=output_directory)
